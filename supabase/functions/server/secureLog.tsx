...
const redactSensitive = (value: any): any => {
  if (typeof value === 'string') {
    ...
    if (value.includes('key') || value.includes('Key') || value.includes('KEY')) {
      ...
      value = value.replace(keyPattern, '***REDACTED_KEY***');
    }
  }
...
export const secureLog = {
  ...
  /**
   * Log database operations (never log query params)
   */
  db: (operation: string, table: string, recordCount?: number) => {
    if (!isDevelopment) return;
    
    if (recordCount !== undefined) {
      console.log(`💾 [DB] ${operation} ${table} - ${recordCount} records`);
    } else {
...
// Export for backward compatibility (replaces console.log in existing code)
